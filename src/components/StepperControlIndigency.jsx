...
const StepperControlIndigency = ({ 
  ...
  const [highlightedStep, setHighlightedStep] = useState(null); // Track the current highlighted step
  const navigate = useNavigate();

  const handleNextClick = async () => {
    if (currentStep === 1 && !uploadIDFieldsComplete) {
      setErrorMessage('Upload Government ID first.');
      setHighlightedStep(1); // Highlight step 1
